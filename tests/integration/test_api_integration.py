...
def main():
    ...
    try:
        # Run tests
        _health_data = test_health()
        _prompt_data = test_system_prompt_upload()
        analysis_data = test_dataset_upload_and_analyze()
        test_session_retrieval(analysis_data['session_id'])
        _storage_data = test_storage_stats()
        _prompts_data = test_list_system_prompts()
        ...
        print("✅ System Prompt Upload & Deduplication: PASS")
        print("✅ Dataset Analysis with Persistence: PASS")
        print("⏭ Session Retrieval: SKIP (not implemented)")
        print("✅ Storage Statistics: PASS")
        print("✅ List System Prompts: PASS")
        print("\n" + "=" * 80)
